    ...
    init(styledMarkdown markdownString: String, fontSize: CGFloat = UIFont.preferredFont(forTextStyle: .body).pointSize) throws {
        ...
        for (intentBlock, intentRange) in s.runs[AttributeScopes.FoundationAttributes.PresentationIntentAttribute.self].reversed() {
            ...
            case .headline(let level):
                switch level {
                case 1:
                    s[intentRange].font = .systemFont(ofSize: 30, weight: .medium)
                    s[intentRange].paragraphStyle = headingParagraphStyle
                case 2:
                    s[intentRange].paragraphStyle = headingParagraphStyle
                    s[intentRange].font = .systemFont(ofSize: 20, weight: .medium)
                case 3:
                    s[intentRange].paragraphStyle = headingParagraphStyle
                    s[intentRange].font = .systemFont(ofSize: 15, weight: .medium)
                default:
...
fileprivate let defaultParagraphStyle: NSParagraphStyle = {
    ...
    paragraphStyle.paragraphSpacing = 30.0
    paragraphStyle.minimumLineHeight = 20.0
    return paragraphStyle
}()

fileprivate let headingParagraphStyle: NSParagraphStyle = {
    var paragraphStyle = NSMutableParagraphStyle()
    paragraphStyle.paragraphSpacing = 100.0
    paragraphStyle.paragraphSpacingBefore = 100.0
    paragraphStyle.minimumLineHeight = 20.0
    return paragraphStyle
}()
